#!/usr/bin/env python3
"""
memory-reconciler: append_memory_log — structured telemetry writer
...
Usage:
    python3 append_memory_log.py \\
      ...
      --event run_completed \\
      --mode scheduled \\
      --details-json '{"sources_ingested": 3, "compile_status": "ok"}'
...
def _generate_run_id(ts: str) -> str:
    ...
    return f'recon-{ts_clean}-{suffix}'
...
def resolve_telemetry_root(cli_dir: str = None) -> str:
    ...
    for env_var in ('RECONCILER_TELEMETRY_ROOT', 'MEMORY_TELEMETRY_ROOT'):
...
def main():
    parser = argparse.ArgumentParser(
        description='Memory Reconciler: Unified Memory Telemetry Writer'
    )
    parser.add_argument(
        '--telemetry-dir',
    ...
    parser.add_argument(
        ...
        help='Run mode: scheduled, manual, first-reconciliation'
    )
    parser.add_argument(
        '--details-json',
        help='JSON string with run details (sources_ingested, compile_status, etc.)'
    )
    parser.add_argument(
        '--error',
